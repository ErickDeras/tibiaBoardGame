...
function formatLine(e: CombatLogEntry): string {
  ...
  switch (e.type) {
    ...
    case "CREATURE_ABILITY":
      return `[Criatura ${e.type === "CREATURE_ABILITY" ? "habilidad" : "ataque"}] ${String(p.attackerId)} (${pos(p.attackerPos)}) → ${String(p.targetId)} (${pos(p.targetPos)}) atk ${String(p.attackValue)} daño ${String(p.damage)} HP ${String(p.hpBefore)}→${String(p.hpAfter)} mana ${String(p.manaCost ?? 0)}`;
    case "COMBAT_MOVE":
